"""
Финальная модель с ensemble и data augmentation для достижения 70-90% F1
"""
...
class DataAugmentation:
    """Data augmentation for text and images"""
...
class EnsembleModel(nn.Module):
    """Ensemble of multiple fuzzy models"""
    
    def __init__(self, models):
        super().__init__()
        self.models = nn.ModuleList(models)
        self.ensemble_weight = nn.Parameter(torch.ones(len(models)) / len(models))
    ...
    def forward(self, text_tokens, image_features, return_explanations=False):
        ...
        for model in self.models:
            result = model(text_tokens, image_features, return_explanations)
            predictions.append(result['binary_logits'])
            if return_explanations:
                explanations.append(result.get('explanations', {}))
        
        # Weighted ensemble
        ensemble_logits = torch.stack(predictions, dim=0)
        weights = torch.softmax(self.ensemble_weight, dim=0)
        final_logits = torch.sum(ensemble_logits * weights.view(-1, 1, 1), dim=0)
        
        result = {'binary_logits': final_logits}
        if return_explanations:
            result['explanations'] = explanations[0]  # Use first model's explanations
...
def plot_confusion_matrix(cm, save_path):
    ...
    plt.title('Confusion Matrix - Final Ensemble Model', fontsize=16, fontweight='bold')
...
def main():
    ...
    print("🚀 ФИНАЛЬНАЯ МОДЕЛЬ: Ensemble + Data Augmentation")
    print("Цель: F1 = 70-90% для статьи")
    print("=" * 70)
    ...
    # Create data loaders
    train_loader = DataLoader(train_dataset, batch_size=6, sampler=sampler, collate_fn=collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=6, shuffle=False, collate_fn=collate_fn)
    
    # Create ensemble of models
    print("\n🤖 Creating ensemble of fuzzy models...")
    models = []
    ...
    print(f"Ensemble created with {sum(p.numel() for p in ensemble_model.parameters()):,} parameters")
    
    # Setup training
    optimizer = optim.AdamW(ensemble_model.parameters(), lr=0.0003, weight_decay=0.01)
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_preds, train_labels, train_probs = train_epoch(
            ensemble_model, train_loader, optimizer, criterion, device, l2_reg=0.001
        )
        ...
        if val_metrics['f1'] > best_val_f1:
            ...
            }, models_dir / 'best_final_ensemble_model.pth')
    ...
    if (models_dir / 'best_final_ensemble_model.pth').exists():
        ensemble_model.load_state_dict(torch.load(models_dir / 'best_final_ensemble_model.pth')['model_state_dict'])
    ...
    with open(results_dir / 'final_ensemble_training_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    # Plot training curves
    plot_training_curves(
        train_losses, val_losses, train_f1s, val_f1s,
        results_dir / 'final_ensemble_training_curves.png'
    )
    
    # Plot confusion matrix
    plot_confusion_matrix(
        np.array(final_metrics['confusion_matrix']),
        results_dir / 'final_ensemble_confusion_matrix.png'
    )
    
    print(f"\n🎉 Final ensemble training completed!")
    print(f"Best model saved to: {models_dir / 'best_final_ensemble_model.pth'}")
    print(f"Results saved to: {results_dir / 'final_ensemble_training_results.json'}")
    ...
    if final_metrics['f1'] >= 0.90:
        ...
        print(f"\n✅ TARGET ACHIEVED! F1 = {final_metrics['f1']:.4f} >= 0.70")
    else:
        print(f"\n⚠️ Target not reached. F1 = {final_metrics['f1']:.4f} < 0.70")
